...
public class RFSteamHammerTE extends CommonTE implements ITickable, ICapabilityProvider {
    public int delta = -1;
    ...
    public CustomEnergyFE energy = new CustomEnergyFE(RFTTConfig.MAX_POWER_STEAMHAMMER, RFTTConfig.MAX_RECIEVE_STEAMHAMMER);

    public RFSteamHammerTE( ) {
        super(1);
        ...
        waterTank = new FluidTank(10000){
            ...
            protected void onContentsChanged() {
                ...
                if(!world.isRemote){
                    RFSteamHammerTE.this.markDirty();
        ...
        verticaalItemHandler = new SidedItemHandler(null, outputHandler, 1);
    }
    ...
    public void update() {
        ...
        if((workCountdown > 0 || currentPistonPosition < 50) && steamTank.getFluidAmount() >= 100 * workCountdown) {
            if (delta != 0) {
                ...
                if(energy.getEnergyStored()>burnTime){
                    energy.extractEnergyInternal(250,false);
                    temperature--;
                }
            }
        } else
